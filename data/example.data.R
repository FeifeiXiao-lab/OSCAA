...
# pdf("cluster.plot.pdf",height=5,width = 7)
# OSCAA.plot(OSA.PCA.res=CNVR.res[["sig.CNR.test"]][[1]])
# dev.off()
...
# example.data=list(example.lrr=signal.mat,
#                   position   =position,
#                   clinic.info=example.data$clinic.info)
setwd("C:/Users/xuanxuan/Dropbox/2021_Researches/20210326_OneStageAssociation/Code/OSCAA R package/example data")
# save(example.data,file = "example.data.Rdata")

example.data<-get(load("example.data.Rdata"))
...
plot(CNVR.res[["candidate.CNR.test"]][[1]][["signal.dred"]],col=factor(CNVR.res[["candidate.CNR.test"]][[1]][["sample.cluster"]]))
plot(CNVR.res[["candidate.CNR.test"]][[1]][["signal.dred"]],col=factor(initial.pis))





table(CNVR.res[["candidate.CNR.test"]][[1]][["sample.cluster"]])


position  =example.data$position
signal.mat=example.data$example.lrr
phenotype=example.data$clinic.info$disease
Dim.reduction="PCA"
assumption="deletion"
...
signal.mat = tmp.signal.mat
phenotype               = phenotype
label.initial           = label.initial
covariates              = covariates
covariates.names        = covariates.names
