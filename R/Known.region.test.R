...
OSA.2D.ECM<-function(
  ...
  phenotype,
  phenotype.type,      # type of phenotype: binary or continuous
  nCNV=NULL,
  ...
  assumption="binary", # assumption between copy number and disease: binary for CNV vs normal; deletion for deletion model; constant for linear relationship between disease risk and copy number 
  ...
  repeat{
    ...
    if (phenotype.type=="continuous"){
      # phe.model<-glm(tmp.phenotype.y ~ design.phe, family = gaussian(link = "identity"),weights = pis)
      # prob.phe <-predict(phe.model,type = "response")
      phe.model<-lm(tmp.phenotype.y ~ design.phe, weights = pis)
      pred.mean <-predict(phe.model,type = "response")
      pred.sd<- summary(phe.model)$sigma
      prob.phe  <- dnorm(tmp.phenotype.y,mean = pred.mean,sd=pred.sd)
      # prob.phe  <- dnorm(tmp.phenotype.y,mean = pred.mean,sd=rep(pred.sd,length(pred.mean)))
    }
  ...
  if (assumption=="constant"){
    ...
    if (phenotype.type=="continuous"){
      p1<-lm(tmp.phenotype.y ~ tmp.design, weights = pis)
      tmp.summary<- summary(p1)
      p.val<-tmp.summary$coefficients[2,"Pr(>|t|)"]
      beta<-tmp.summary$coefficients[2,"Estimate"]
      std <-tmp.summary$coefficients[2,"Std. Error"]
    }
  ...
  if (assumption=="binary"){
    ...
    if ( !is.null(covariates.pheno.names) ){
      ...
      colnames(tmp.design)[(ncol(tmp.design)+1-length(covariates.pheno.names)):ncol(tmp.design)]<-covariates.pheno.names
      tmp.design<-as.matrix(tmp.design)
    }else {tmp.design<-phe.CNV.state.bin}
    if (phenotype.type=="binary"){
      p1<-glm(tmp.phenotype.y ~ tmp.design, family = binomial(link = "logit"),weights = pis)
      p.val<-summary(p1)$coefficients[2,"Pr(>|z|)"]
      beta<-summary(p1)$coefficients[2,"Estimate"]
  ...
  if (assumption=="deletion"){
    ...
    if (any(phe.CNV.state.del==1)){
      if ( !is.null(covariates.pheno.names) ){
        ...
        colnames(tmp.design)[(ncol(tmp.design)+1-length(covariates.pheno.names)):ncol(tmp.design)]<-covariates.pheno.names
        tmp.design<-as.matrix(tmp.design)
      }else {tmp.design<-phe.CNV.state.del}
